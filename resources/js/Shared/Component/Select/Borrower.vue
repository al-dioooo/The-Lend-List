  ...
  <Listbox as="div" v-model="selected">
    ...
    <div class="relative mt-1">
      <ListboxButton class="relative w-full py-2 pl-3 pr-10 text-left transition bg-white border border-gray-300 rounded-md shadow-sm cursor-default focus:outline-none focus:ring-2 focus:ring-opacity-60 focus:ring-blue-300 focus:border-blue-500 sm:text-sm">
          <span :class="{ 'text-gray-500' : !selected }" class="block truncate" v-html="selected ? selected.name : 'Select borrower'"></span>
          <span class="block text-gray-500 truncate" v-html="selected ? selected.email : ''"></span>
      ...
      <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <ListboxOptions class="absolute z-10 w-full p-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            ...
            <li :class="[active ? 'text-white bg-gray-800' : 'text-gray-900', 'cursor-default select-none relative py-2 pl-3 pr-9 rounded-md']">
              <div class="flex flex-col">
                ...
                <span class="text-gray-500">
                  {{ row.email ? row.email : row.contact }}
                </span>
              </div>
